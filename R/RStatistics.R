#Saphiro.test() sirve para determinar la normalidad de una distribución 
#ks.test() para ver si dos muestras corresponden a la misma población

# UNA MUESTRA T-TEST
set.seed(100)  #creamos una distribución normal aleatoria con rnorm 50=n datos, media 10 y desviación 0.5
...
anova <- aov(Vigilancia~Dosis,datos_anova)  #vigilancia-dtos significa que la vigilancia depende de dosis
summary(anova)
...
kruskal.test(n_huevos ~ condicion, data = datos) 	#como anova pero para datos que no son normales
...
# FISHER. usar cuando las tablas son pequeñas
var.test(x,y)
fisher.test(matrix)
